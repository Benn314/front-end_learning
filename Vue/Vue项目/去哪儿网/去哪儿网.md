...
![image-20220907233146602](去哪儿网.assets/image-20220907233146602.png)

![image-20220907233155798](去哪儿网.assets/image-20220907233155798.png)

![image-20220907233203501](去哪儿网.assets/image-20220907233203501.png)
...
![image-20220908084831418](去哪儿网.assets/image-20220908084831418.png)
...
包括工具类还有其他webpack的配置项都是这个目录里 这个目录是VueCLI 自动帮我们构建好的webpack的集合 一般来说我们也不需要修改 放着就好了
...
![image-20220908155054148](去哪儿网.assets/image-20220908155054148.png)
...
移动端有一个300ms的点击延迟问题 导入fastClick包来解决问题
...
有一个问题 swiper-container跟 .icons 的范围不一样 而滑动的范围是由swiper-container决定的 也就是说 icons除了swiper-container的其他范围无法做出滑动翻页的效果 我们需要将swiper-container的范围设置跟icons范围等同
...
![image-20220911131142091](去哪儿网.assets/image-20220911131142091.png)
...
当我们项目上线时 数据请求路径并不是来自static/mock的我们这个本地模拟的接口地址，而是改成api的这种格式 当我们准备上线项目时 对项目代码进行改动是有风险的
...
![image-20220911162122162](去哪儿网.assets/image-20220911162122162.png)
...
![image-20220912110334208](去哪儿网.assets/image-20220912110334208.png)
...
向Search.vue传入data 并设置watch监听 增加了搜索结果样式 在computed计算属性中增加布尔类型函数 做v-if/v-show的属性值判断是否显示

麻烦的点在于这里要嵌套for（在定时器setTimeout里）

并且显示结果过多无法显示页面一下部分（无法拖拽滚动余下的结果）这里用better-scroll解决 导入并在mounted里创建实例即可

​	

## 8-9 使用Vuex实现数据共享

用我们之前bus总线的方式也能实现数据共享，但vue官方提供了更好的数据共享工具`vuex`，vuex是数据框架，可以共享数据（官方推荐）

这里我们需要两个页面进行数据交互 所以需要数据共享

> 业务场景之一：在city页中选择城市后，首页右上角显示的是city页选择的城市 根据城市不同 在首页展示该城市特色
...
这一部分是属于各个组件或页面的公共部分，我们可以把它理解为一个stock仓库（stock：库存），这一个仓库由三个部分组成：Actions（行为）、Mutations（变化；转变）、State（状态）

我们所有的公共数据都存储在state当中，那如果有一个组件想用公共数据，直接去调用state就可以了；而如果我们需要改变state的数据，我们不能直接让组件components直接改变state共有数据，必须走一个流程，这个流程是这样的：如果我有一些异步操作，那么我把异步操作放在Actions这里，或者一些重复操作/批量操作也放在Actions这里，组件components先去调用Actions，Actions紧接着去调用Mutations，Mutatuons放着的是一个一个同步的对State的修改。当然这也不是绝对的，有时候我们可以越过Actions，直接让components调用Mutations对state进行修改

components调用Actions采用的是Dispatch（派遣；发送）方法，Actions调用Mutations调用的是commit方法，是一个单向数据的改变流程，而在做Mutations的时候，我们还可以借助我们的vue Devtools做调试

​	

在访问根实例的时候 我们已经把store这个属性传递进去了 每个子组件都会收到我们这个属性

![image-20220914210508088](去哪儿网.assets/image-20220914210508088.png)

​	

因为只是修改一个state值 没有异步操作和批量操作 没必要转发dispatch调用actions，直接用mutations调用commits就行了

[Vue router 编程式导航](https://router.vuejs.org/zh/guide/essentials/navigation.html#%E7%BC%96%E7%A8%8B%E5%BC%8F%E5%AF%BC%E8%88%AA) 实现页面点击事件后的跳转联动

​	

![image-20220914230428504](去哪儿网.assets/image-20220914230428504.png)

​	

## 8-10 Vuex的高级使用及localStorage

localStorage 本地存储的内容

localStorage 的API比cookie更加简单 所以我们这里采用localStorage完成我们想要保存的城市保存的功能

但是大家要记得，当你使用localStorage的时候 我建议大家只要你用它就在最外层包裹一个try-catch 因为在某些浏览器 如果用户关闭了本地存储功能或者使用隐身模式 你使用localStorage可能会使浏览器抛出异常 你代码就整个运行不了了 而为了避免出现这种问题 我们建议在最外层加一个try-catch

​	

... 是展开运算符

mapState是指我把vuex里的数据映射到我这个组件的computed计算属性里，那映射什么呢？我把city这个公用数据映射到一个叫做city的计算属性之中

传递给mapState可以是一个数组也可以是一个对象

可以调用vuex的mapMutations进而修改state值 记得导入 import {mapMutations, mapState} from 'vuex'

​	

除了state、mutations、actions，还有getter，getters对应的是一个对象

其实getters在vuex的作用有点像在vue中computed计算属性的作用，如果要用state的数据计算出新的数据，我们就可以借助getters这个属性来进行，这样我们可以避免数据的冗余

什么时候要用到Module呢，其实是这样的，当我们遇到非常复杂的应用场景，例如我们在管理后台数据的时候，经常会有很多功能数据在vuex里面进行存储，如果我们把所有Mutation都放在Mutations进行存储，这个文件将会变得很庞大，难以维护。我们这时可以借助Module对一个复杂的Mutations和state包括actions进行拆分

在创建store对刚刚定义的各个模块进行一个整合就行，每个模块只负责自己相关的部分 通过module来写我们的代码 可以让我们的代码更具维护性 当然了我们现在项目中只有一个数据city，没必要用到module 反而使用它会让我们代码看起来有点别扭
...
![image-20220915103144099](去哪儿网.assets/image-20220915103144099.png)

问题：每一次切换页面的时候 json文件都会被请求一次

这里keepAlive的意思是 我的路由加载了一次之后 我就把我的路由加载到内存之中 下一次再调用这个路由的时候 不需要你去重新渲染这个组件 去重新执行mounted钩子函数 从内存里把以前的东西拿出来显示就可以 就不会再去调用Ajax请求了

---

还有一个问题是（性能进一步优化） 当我们更改了首页右上角的城市选择时，首页也应该更改界面，变成相应城市的界面才对

这里我们要用到mounted和activated，页面被第一次加载的时候，mounted一定会被执行，而activated是页面被(重新)显示的时候 会被执行，我们在Home.vue首页中定义一个临时缓冲变量lastCity中，来对city做一个判断，让最开始的city信息存储在lastCity，当进入城市选择时，我们在activated中做一次if判断，因为activated在页面重新显示时便会被执行一次，所以我们选择它，如果选择了跟上一次同样的城市，则信息不变，而如果选择了其他的城市，则改变临时缓冲变量，并重新请求一次Ajax，重新执行getHomeInfo()，重新渲染其json文件，获取最新的首页页面信息
...
## 9-1 详情页动态路由和banner布局

CSS rgba() 函数：使用 RGBA 颜色，并使用透明度：
...
使用新的小图标，除了将图标下载至本地，还要更新iconfont.css里的url地址
...
![image-20220915202209112](去哪儿网.assets/image-20220915202209112.png)
...
画廊点击时 可能会出现滚动不顺畅的情况 原因是计算宽度有问题 在显示渲染的时候 而我们只需要在Gallary.vue中添加observeParent和observer属性，设置为true 进行监听点击事件 当监听到点击事件时 会重新刷新一次页面 计算宽度便得以正常显示

​	

## 9-3 实现header渐隐渐显效果

实现了detail页的hearder部分
...
很重要 很多初学者没有对全局事件进行解绑 项目出现了大量bug
...
因为我们这里是全局监听 事件不是绑定在标签元素内部的(例如标签内@click什么的)，所以会对该标签以外的元素影响，当我们监听完毕时 应该对监听事件进行解绑 这样才不会对其他元素产生影响

> activated生命周期钩子是在他在每次页面显示的时候被执行 
>
> deactivated生命周期钩子是在页面即将被隐藏或替换成新页面的时候被执行

​	

## 9-5 使用递归组件实现详情页列表

之前我们不太清楚我们组件的名字都可以拿来干嘛（除了辨别身份）

![image-20220916104212569](去哪儿网.assets/image-20220916104212569.png)

现在他多了一个用处 组件名字很大用处就是我们使用递归组件的时候 拿来使用的 假设一个组件要递归的时候 就可以拿自己的名字递归自己
...
使用Ajax获取detail.json动态数据，还存在一个问题 之前讲过的缓存问题，由于在App.vue用了keepAlive做了缓存使得detail.json获取过一次后会加入缓存区中，所以例如当我们进入id=0001的页面后退出进入0002页面，控制台的【网络】中并不会显示刷新detail.json文件显示id=0002，而是重新从缓存区中取出数据 显示还是0001，上一次我们是设置了activated的生命周期钩子使其切换页面后重新刷新获取新json数据，而这一次我们采用另一种方式

假设我想让我们详情页面无需做缓存，可不可以呢？完全可以

> exclude  （故意）不包括，把……排除在外；

直接在keepAlive标签中设置exclude属性，把detail组件页面排除再外 不加入缓存 这样每次进入该页面都会执行mounted生命周期钩子，刷新一次json文件数据重新获取新数据，解决缓存带来的问题
...
1. 组件做递归操作时用到
2. 组件设置是否缓存时用到
3. Vue Devtools工具dom树里的名称显示取决于你的组件名称

​	

而现在还有一个问题 就是你再A页面滑动到X位置，你跳转到项目的B页面，B页面不会在页面顶部显示，而是显示B页面的X位置，所以页面之间的滑动是会相互影响的，我们要把这个问题解决了

![image-20220916140355831](去哪儿网.assets/image-20220916140355831.png)

我们在vue官方文档中点击点击vue router查看滚动行为 复制切换页面默认滚动到顶部的代码
...
复制到项目的/router/index.js中去 便可解决该问题
...
在Banner.vue引入  <fade-animation>组件使得点击画廊图片的时候图片有渐隐渐显的效果

而这里存在的一个bug 是点击画廊图片渐显放大时 下方文字不能及时隐去 存在图片上一段时间后才显示 不知如何解决

​	

# 第十章 实战项目 项目的联调，测试与发布上线

## 10-1 Vue项目的接口联调

我们在此之前做的都是前端模拟接口的数据（至此前端的内容已开发完毕，模拟数据在mock目录下） 而现在我们可以尝试用后端返回的数据来进行调试，而这也叫**接口联调**

http://localhost/api/index.json 这么写默认是80端口

http://localhost:80/api/index.json

后台服务器端口默认在80端口上 所以这里还可以直接省略80端口不写，默认是

.config/index.js

![image-20220916192110646](去哪儿网.assets/image-20220916192110646.png)

![image-20220916192146894](去哪儿网.assets/image-20220916192146894.png)

![image-20220916192528093](去哪儿网.assets/image-20220916192528093.png)

在实际开发中呢，后端的服务器可能没在我们自己电脑上，而是在其他开发人员的电脑上，那么这时我们就不能用本地localhost了，而是要用内网的ip地址，或者外网的域名 都是可以的

在公司里面如果想实现vue的前后端联调，不需要再使用类似于fiddler、charles这样的抓包代理工具了，只需要使用proxyTable这个配置项，把你需要请求的后端的服务器地址写在这里就完全OK了

​	

## 10-2 Vue项目的真机测试

通过cmd输入ipconfig（mac是ifconfig）查看本机的ip地址，等同于localhost

我们在浏览器可以通过localhost访问我们的这个前端项目，而通过ip地址也是等同的，不过他此时拒绝访问，原因是：

![image-20220916193310639](去哪儿网.assets/image-20220916193310639.png)

我们前端的项目是通过webpack-dev-server启动的，而webpack-dev-server默认不支持通过ip的形式进行页面的访问，所以我们需要把他默认的配置项进行修改

![image-20220916193636946](去哪儿网.assets/image-20220916193636946.png)

然后关闭项目 重启服务器试一下ip地址能不能访问到

![image-20220916193736261](去哪儿网.assets/image-20220916193736261.png)

​	

通过手机测试后 我们可以发现在搜索页 拖动右侧字母表ABCD，页面会跟着上下拖动

![image-20220916195536008](去哪儿网.assets/image-20220916195536008.png)

在city/components/Alphabet.vue中修改这段代码为

![image-20220916195942717](去哪儿网.assets/image-20220916195942717.png)

叫做事件修饰符 他可以阻止touchstart的默认行为，页面就不会上下拖动了

​	

真机测试，通过设置ip地址可以访问vue项目后，我们便可以在手机端或模拟器上进行功能测试了

![image-20220916200423857](去哪儿网.assets/image-20220916200423857.png)

这是iphone 6s的手机模拟器测试的 如果是低版本的安卓手机上访问 可能会出现白屏的效果，原因有二种：

1. 你的手机浏览器不支持proxy代理，解决这个问题只需要安装![image-20220916200716332](去哪儿网.assets/image-20220916200716332.png)

babel-polyfill会判断你手机浏览器有没有proxy，自动帮你添加相关配置

![image-20220916200854729](去哪儿网.assets/image-20220916200854729.png)

然后在main.js引入这个包，这样我们这个项目就可以在任何浏览器上支持proxy啦

![image-20220916201010862](去哪儿网.assets/image-20220916201010862.png)

这样一部分同学手机展示白屏问题就可以得到解决（待会在自己手机上试验一下~）如果还出现白屏，这不是代码问题，而是webpack-dev-server的问题，不用着急，下节课会讲webpack的打包上线，将项目打包好了之后，放入真正的环境或者线上环境上做一个测试，基本将代码上线到服务器或线上服务器的时候，白屏问题就不会出现了
...
进入vue项目目录 将代码真正放到线上该怎么做呢 将src代码给到后端服务器然后就可以访问到我们项目吗？不是这样。当我们做vue项目上线的时候，

第一步

![image-20220916202521039](去哪儿网.assets/image-20220916202521039.png)

使用npm run build ，vue的脚手架工具会自动对src下的源代码进行打包编译，生成一个能被浏览器运行的代码，同时这个代码也会是一个压缩后的代码

![image-20220917004113810](去哪儿网.assets/image-20220917004113810.png)

显示Build complete说明打包已经完成

这是进入项目根目录会发现多出了一个dist文件夹目录

![image-20220917102108052](去哪儿网.assets/image-20220917102108052.png)

放到后台服务器的根目录下

![image-20220917102323068](去哪儿网.assets/image-20220917102323068.png)

这里现在再访问的localhost是后端服务器，端口是80端口

![image-20220917102415399](去哪儿网.assets/image-20220917102415399.png)

扔到后端项目里，还有后端服务器提供的接口

这样就完成了最简单的项目上线

如果希望把前端打包上线文件放在后端新建文件夹project中，只需要在config/index.js 中将assetsPublicPath: '/'  修改为 assetsPublicPath: '/project'就行了

![image-20220917103035170](去哪儿网.assets/image-20220917103035170.png)

然后重新运行一次npm run build进行打包 将生成新的dist和index.html文件放到后端项目根目录下就可以了

将dist重命名为project放到后端项目根目录下

![image-20220917103243984](去哪儿网.assets/image-20220917103243984.png)
...
插件和自定义指令讲得少，后续可在官网文档继续学习，对边缘知识要学习一下，提升边缘知识点的能力

vue rounter讲的是主要的一个 动态路由匹配，还有很多重要的没讲到，命名路由、重定向和别名、路由导航守卫等 这些应该进入官方文档仔细做一个阅读，这样对vue rounter也就是前端路由有一个深刻的认识

vuex讲得多，核心概念都涉及了，有一些知识点也需要后面线下自己去做一个补充，例如大型复杂项目的时候，我们需要去官方文档看一下项目结构应该怎么设计   还有如何使用vuex的一些插件 这些都可以去官方文档做一个仔细阅读做研究

上面的vue rounter和vuex都没问题的话，可以去看一下现在比较流行的 vue服务器端渲染 这一个会相对复杂 单独拿出来做一节课都不为过，如果服务器端渲染也搞明白了，那么对vue node.js vue rounter vuex都没问题  如果是新人，对前后端分得不清楚的话 可以延迟学习这部分的内容

当我们把vue的基本概念都搞懂了，可以去研究vue的一些非常好用的插件

在这里 [vue 插件地址](https://github.com/vuejs/awesome-vue)

![image-20220917105236224](去哪儿网.assets/image-20220917105236224.png)

可以去研究不同插件的源码，研究如何去使用，再后来如果对vue了解都很深刻了，可以去下载vue的源码进行研究，也可以去vue的github仓库 git commit对vue的代码进行一个追溯 去看作者是如何一步一步就构建vue这个项目的 在这个过程中可以发现 vue的源码考虑到很多性能上的细节点，而且代码也写得比较漂亮 在阅读过程中呢 相信大家也能很大的提升自己代码的编程能力 vue这些都研究很明白后，还可以研究webpack babel es6这些方面的内容

这门课程只是入门vue的敲门砖 希望大家往后继续深入学习 后面有缘再会！
